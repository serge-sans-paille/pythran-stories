...
one. This slightly impacts the code size and eventually gives the compiler more
optimization opportunities, but nothing significant there (131K), apart the pleasure
of generating cleaner binaries. That's also going to be the default for next
Pythran version.
...
consider the current compilation unit (or code) as a whole program. ``main`` is
still the entry point, but one can set extra attributes to function to flag
them as exported too, using ``__attribute__ ((externally_visible))``. That
sounds a bit redundant to me with the visibility attribute, but it turns out
this triggers abunch of different optimization path that gives us a significantly smaller
binary, that runs slightly faster:
...
Combining ``-O3`` and ``-march=native`` triggers compiler auto-vectorization,
...
triggers some hard-coded vectorization based on ``boost.simd`` [2]_, and that
**did** help::
...
.. [2] Thanks Numscale https://www.numscale.com/boost-simd/
